...
export class FSM {
    ...
    _finalize() {
        ...
        // **** YOUR CODE HERE ****
        // double forloop through states and transitions
        // bind transitions to actual target states
        for (const state of this.states) {
            for (const transition of state.transitions) {
                // bind target
                transition.bindTarget(this.states);
                // bind event
                const eventSpec = transition.onEvent;
                if (eventSpec.regionName !== '*') {
                    // find region with matching name
                    const region = this.regions.find(reg => reg.name === eventSpec.regionName);
                    // if not
                    if (!region) {
                        // then throw error
                        Err.emit(`Region ${eventSpec.regionName} not found`);
                        // else bind the region
                    }
                    else {
                        eventSpec.bindRegion([region]);
                    }
                }
                // bind actions to regions
    ...
    actOnEvent(evtType, reg) {
        ...
        for (const transition of this.currentState.transitions) {
            if (transition.match(evtType, reg)) {
                // for each action in actions, execute the action and move to target state
                for (const action of transition.actions) {
