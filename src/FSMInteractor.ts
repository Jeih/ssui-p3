...
export class FSMInteractor {
    ...
    public pick (localX: number, localY: number): Region[] {
        ...
        // Check each region in reverse order
    ...
    protected _currentRegions: Region[] = [];
    ...
    public dispatchRawEvent (what: 'press' | 'move' | 'release',
        ...
        // **** YOUR CODE HERE ****
        // Get the new pick list for the current position
        const newPickList = this.pick(localX, localY);

        // Handle exits first - check for regions we were in but aren't anymore
        for (const oldRegion of this._currentRegions) {
            if (!newPickList.includes(oldRegion)) {
                this.fsm.actOnEvent('exit', oldRegion);
            }
        }

        // Handle enters next - check for regions we weren't in but are now
        for (const newRegion of newPickList) {
            if (!this._currentRegions.includes(newRegion)) {
                this.fsm.actOnEvent('enter', newRegion);
            }
        }
        ...
        switch (what) {
            case 'press':
                // Press events for each picked region
                for (const region of newPickList) {
                    this.fsm.actOnEvent('press', region);
                }
                break;

            case 'move':
                // Move_inside events for each picked region
                for (const region of newPickList) {
            ...
            case 'release':
                if (newPickList.length > 0) {
                    // Release events for each picked region
                    for (const region of newPickList) {
                        this.fsm.actOnEvent('release', region);
                    }
                }
                else {
                    // Release with no region picked
                    this.fsm.actOnEvent('release_none');
                }
                break;
        }

        // Update current regions for next time
        this._currentRegions = newPickList;
