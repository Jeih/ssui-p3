...
export class EventSpec {
    ...
    public bindRegion(regionList : readonly Region[]) : void {
        ...
        // **** YOUR CODE HERE ****
        // find the region object with the given name
        const found = regionList.find(r => r.name === this._regionName);
        if (found) {
            this._region = found;
            return;
        }
        ...
        Err.emit(`Region '${this._regionName}' does not match any region.`);
    }
    ...
    public match(evtType : EventType, regn? : Region) : boolean {
        ...
        // **** YOUR CODE HERE ****
        // if event type is nevermatch then it never matches
        if (this._evtType === 'nevermatch') return false;
    
        // match any region
        if (this._evtType === 'any') {
            // If we have "*" region name
            if (this._regionName === "*") return true;
            // else it is the region
            return this._region === regn;
        }
        // if it does not match the event type then it does not match
        if (this._evtType !== evtType) return false;
    
        // If region name is "*" then match any region
        if (this._regionName === "*") return true;
        // Else, match the specific region
        return this._region === regn;
    }   
